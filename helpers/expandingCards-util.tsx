...
export const isActive: MouseEventHandler<HTMLImageElement> = (event) => {
  const id = getId(event.currentTarget);
  removeActive();
  addActive(id);
};

const getId = (currentTarget: EventTarget & HTMLImageElement) => {
  const id: string = currentTarget.getAttribute("id")!.replace("image-", "");
  return id;
};
